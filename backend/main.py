...
Answer:"""
        ...
        # Call Ollama LLM with retries/backoff and robust parsing
        answer = None
        last_exception = None
        max_attempts = 3
        timeout_seconds = 600  # generous timeout while model may be loading
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                # Try parsing response JSON; Ollama's shape can vary by version
                try:
                    data = resp.json()
                except Exception:
                ...
                # Common places the generated text may live
                ...
                # Fallback
                if not answer:
                    # If no recognized key, stringify a few useful fields for debugging
                    possible = []
                ...
                # Successfully got an answer or fallback text
                break
            except requests.exceptions.Timeout as e:
                last_exception = e
                # If model is cold-starting, wait and retry
